...
const UsersWorkshop = () => {
    ...
    return (
        <div className=' bg-[#351b26]  h-screen'>
            <div className='pt-12 flex gap-2 justify-center items-center'>
                <form onSubmit={getUser}>
                    ...
                    />
                    <button type='submit' className='w-24  text-gray-300 border rounded-lg p-1.5 focus:outline-none focus:ring-1 focus:ring-yellow-200'>Get Details</button>
                </form>
            </div>

            
            <div className="flex flex-wrap justify-center bg-[#351b26] mt-16">
    {user.map((person, index) => (
        <div key={index} className="w-96 mx-4 mb-8 mt-12">
            <div className="rounded-xl bg-gray-300 p-6 text-center shadow-xl">
                ...
                <h1 className="mb-3 text-xl font-medium ">{person.workshop}</h1>
                <ul className='flex flex-col items-start justify-start gap-2'>
                    <li>{person.name}</li>
                    <li>{person.email}</li>
                    <li>{person.paymentId}</li>
                    <li>{person.contact}</li>
                    <li>{person.createdAt ? new Date(person.createdAt).toLocaleDateString('en-IN') : ''}</li>
                </ul>
            </div>
        </div>
    ))}
